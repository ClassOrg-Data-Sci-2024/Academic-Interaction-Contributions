...
Additionally, the [MICASE Corpus Manual](https://ca.talkbank.org/access/0docs/MICASE.pdf) explains in Table 3-2 that words that were "completely unintelligible" were transcribed as two x's in parentheses.
...
## Counting instances of "non-words"

The MICASE Corpus Manual lists all the transcription conventions used for hesitation and filler words, backchannel cues, exclamations, and truncated words which allows all these instances to be counted in a straightforward way.
...
nonword_regexs <- c(hesitation_regex, backchannel_regex, exclamation_regex, truncated_regex)
...
map(nonword_regexs, ~ word_counter(.x))
```
